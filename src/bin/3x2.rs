...
fn main() {
    ...
    let mut numbers = board.find_numbers()
        .into_iter()
        .filter_map(|number| {
            for point in number.get_kernel() {
                match board.get(point) {
                    Some(&x) if x == '*' => return Some((number, point)),
                    _ => continue
                }
            }
            None
        })
        .collect::<Vec<(Number, Point)>>()
        ;
        
    numbers.sort_by_key(|pair| pair.1);
    numbers.reverse();
    let mut total: i32 = 0;
    let mut prev: Option<(Number, Point)> = None;
    for i in 0..numbers.len() {
        let curr = match numbers.pop() {
            Some(x) => x,
            None => continue
        };
        match prev {
            Some(prev) if prev.1 == curr.1 => 
            total += prev.0.value(&board) * curr.0.value(&board),
            _ => prev = Some(curr)
        }
    }
    println!("{total}");
    ...
    // number.value(&board);
    numbers.iter().for_each(|&x| {println!("{:?} \t| Coord: {:?} \t| Gear: {:?}",x.0.value(&board), x.0, x.1);});
